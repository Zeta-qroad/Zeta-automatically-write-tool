...
# 설정을 저장할 파일 경로
settings_file = "zeta_v11.0.3_setting.json"
...
def common_actions():
    ...
    if new_clipboard in invalid_words:
        print(f"⚠ 경고: 붙여넣기 잘못됨! 다시하셈! (현재: {new_clipboard})")
        return  # 잘못된 값이 붙여넣어지는 것을 방지

    pyautogui.hotkey('ctrl', 'v')  # 클립보드 내용이 올바르면 붙여넣기
    time.sleep(0.3)
    
    # 붙여넣기 후 클립보드 내용 확인
    time.sleep(0.2)  # 클립보드 변경 대기
