...
def change_tap_cccv_and_back(expected_clipboard_content, timeout=2.0, max_retries=2, retry_delay=0.5):
    ...
    while attempt <= 2:  # 최대 2회 시도 (초기 + 재시도)
        ...
        if ui_success_result:
            ...
            if attempt == 1:  # 최초 실패 시 재시도
                pyautogui.hotkey('ctrl', 'shift', 'tab')
            attempt += 1

    if attempt > 2:  # 두 번째 시도 실패
