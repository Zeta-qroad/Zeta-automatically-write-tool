...
# 설정을 저장할 파일 경로
settings_file = "zeta_v10.4.1_setting.json"
...
def img_dell_action():
    ...
    pyautogui.click()
    time.sleep(0.8)
    pyautogui.moveTo(settings["imgdc_x"], settings["imgdc_y"] - 15)
    pyautogui.click()
...
def open_settings():
    ...
    settings_window.geometry("300x750")
    ...
    tk.Button(settings_window, text="저장", command=lambda: save_settings_window(start_x_entry, pause_entry, check_c_entry, fil_f_entry, dell_d_entry, hdell_d_entry, imgd_x_entry, imgd_y_entry, imgdc_x_entry, imgdc_y_entry), **local_button_style).pack(pady=9)
    ...
    def save_settings_window(start_x_entry, pause_entry, check_c_entry, fil_f_entry, dell_d_entry, hdell_d_entry, imgd_x_entry, imgd_y_entry, imgdc_x_entry, imgdc_y_entry):
        try:
