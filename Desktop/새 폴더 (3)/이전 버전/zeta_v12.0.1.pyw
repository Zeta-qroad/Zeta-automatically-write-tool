...
def on_tab2_button2_click():# 노출 차단
    settings["button1_clicks"] += 1
    perform_action("검색차단", settings["block_x"])

def on_tab2_button3_click(): # 삭제
    settings["button1_clicks"] += 1
    perform_action("삭제", settings["del_x"])

def on_tab2_button4_click(): # 삭제 및 차단
    settings["button1_clicks"] += 1    
    perform_action("차단삭제", settings["block_del_x"]) 
...
def open_settings():
    ...
    def save_settings_window():
        try:
            # 기본 좌표값 가져오기
            ok_x = int(ok_x_entry.get())
            img_block_x = int(img_block_x_entry.get())
            ...
            # 계산된 좌표 설정
            settings["ok_x"] = ok_x
            settings["block_x"] = block_x
            settings["del_x"] = del_x
            settings["block_del_x"] = block_del_x
            settings["img_block_x"] = img_block_x
